...
##            : 1.03 reMap hotkey
##            : 1.11 fixing bug where rotation caused mesh not algin to surface nomral
##                   removed some hotkey as it's not useful
##                   no crash in linux system, for some reason is slower than windows 
##                   rmove pymel code to avoid extra error.
##            : 1.12 adding click mode
##            : 1.13 stepping rotation ( more useful~ )
##            : 1.14 merge align edge function 
##            : 1.15 auto pivot to base of mesh  
##            : 1.16 fixing bug with align edge when multi mesh structure selected 
##            : 1.17 fixing bug with raytrace return wrong face id when multi intersection found 
##			  : 1.2  public test
##            : 1.3  add Aim mode
## Install    :
...
def instDragPick():
    ...
    global recordBasePosi
    recordBasePosi = [0,0,0]
    edgeAlignRecord = 0
    storeRotCount = 0
...
def instDragClean():
    ...
    cleanList = ('instPicker','instRot','aimLoc')
    for c in cleanList:
        if mc.objExists(c):
            mc.delete(c)
    instDul = 0
        ...
        if mc.objExists('instPicker'):
            ...
            if (modifiers == 8):
                ...
                if finalMesh:
                    ...
                    if mc.objExists('aimLoc') == 0:
                        mc.spaceLocator(p = [0,0,0],n='aimLoc')
                    if mc.objExists('pickerAim') == 0:
                        mc.aimConstraint("aimLoc", "instPicker",
                            offset=[0, 0, 0], 
                            weight=1, 
                            aimVector=[0, 1, 0], 
                            upVector=[0, 1, 0], 
                            worldUpType="vector", 
                            worldUpVector=[0, 1, 0], n='pickerAim')
                    mc.setAttr('aimLoc.translate', finalX,finalY,finalZ)
                    mc.refresh(cv=True,f=True)
      
            elif (modifiers == 4):
                if mc.objExists('pickerAim'):
                    mc.delete('pickerAim')
                #press Ctrl ----> rotate
                if screenX > vpX:
                ...
                if finalMesh:
                    ...
                    instDul = 0
                    mc.setAttr('instPicker.translate', finalX,finalY,finalZ)
                    recordBasePosi = [finalX,finalY,finalZ]
