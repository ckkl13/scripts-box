...
class GroupPanel:
    ...
    def rename_group(self, group):
        """重命名组：重命名磁盘文件夹、更新 id/name、更新所有引用和 groups_order.json"""
        old_id = group["id"]
        old_name = group["name"]
        ...
        old_group_dir = os.path.join(self.parent.tools_dir, old_name)
    ...
    def delete_group(self, group):
        ...
        # 准备在回收站中的目标路径
        recycle_group_dir = os.path.join(self.parent.recycle_dir, group_name)
        
        # 处理回收站中同名文件夹的情况
        if os.path.exists(recycle_group_dir):
            counter = 1
            while os.path.exists(os.path.join(self.parent.recycle_dir, f"{group_name}_{counter}")):
                counter += 1
            recycle_group_dir = os.path.join(self.parent.recycle_dir, f"{group_name}_{counter}")
        
        try:
            # 移动整个分组文件夹到回收站
            shutil.move(group_dir, recycle_group_dir)
            ...
            deleted_group = {
                ...
                "folder_name": os.path.basename(recycle_group_dir),
                "tools": tools_in_group.copy()
            }
            ...
            # 将删除的分组添加到回收站
            self.parent.config["recycle_bin"].append(deleted_group)
            
        except Exception as e:
            cmds.warning(f"删除分组失败: {group_dir}, 错误: {str(e)}")
        ...
        cmds.inViewMessage(message=f"已删除分组 '{group_name}'，已移入回收站", pos='midCenter', fade=True)
    
    def create_nav_button(self, group):
        """创建导航栏按钮（或列表界面下向列表添加一项）"""
        group_id = group["id"]
        group_name = group["name"]
        
        # 当前界面为列表导航（无 nav_buttons_layout）时，向列表添加新项即可
        if not getattr(self.parent, 'nav_buttons_layout', None):
            item = QListWidgetItem(f"{group_name} (0)")
            item.setData(Qt.UserRole, group_id)
            self.parent.nav_list.addItem(item)
    ...
    def on_nav_tree_context_menu(self, pos):
        """列表导航右键菜单"""
        item = self.parent.nav_list.itemAt(pos)
        if not item:
            return
        ...
        # 显示右键菜单
        self.show_nav_context_menu(group, pos, button or self.parent.nav_list)
