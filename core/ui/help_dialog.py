...
class HelpDialog:
    ...
    def add_to_shelf(self, tool):
        """将脚本添加到Maya工具架"""
        try:
            # 获取当前活动的工具架 - 使用Maya全局变量 $gShelfTopLevel
            try:
                shelf_top_level = mel.eval('$temp = $gShelfTopLevel')
            except Exception:
                shelf_top_level = "ShelfLayout"
            
            if not shelf_top_level or not cmds.tabLayout(shelf_top_level, exists=True):
                cmds.warning("无法获取当前工具架")
                return
            
            current_shelf = cmds.tabLayout(shelf_top_level, query=True, selectTab=True)
            if not current_shelf:
                cmds.warning("无法获取当前工具架标签页")
                return
            
            # 获取脚本文件路径 - 优先使用tool中已有的path字段
            tool_path = tool.get("path", "")
            if not tool_path or not os.path.exists(tool_path):
                # 如果path字段不可用，尝试通过group和filename构建
