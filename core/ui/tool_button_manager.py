...
class ToolButtonManager:
    ...
    def create_tool_button(self, tool, group_id=None):
        ...
        try:
            ...
            if os.path.exists(icon_path):
                ...
                # 图标从18px缩小到14px左右（缩小0.9倍）
                button.setIconSize(QSize(int(18 * 0.9), int(18 * 0.9)))
        except Exception:
            pass
    ...
    def show_context_menu(self, position, button, tool):
        ...
        try:
            ...
            try:
                color_action.setIcon(self.parent.style().standardIcon(QStyle.SP_DialogSaveButton))
            except:
                pass
    ...
    def apply_button_color(self, button, tool):
        ...
        try:
            ...
            from core.utils.qt_compat import get_scaled_font_size
            
            # 使用动态缩放的字体大小，比基准小0.9倍（缩小约10%）
            base_font_size = 14 * 0.9  # 基准字体大小缩小0.1
            scaled_font_size = get_scaled_font_size(base_font_size)
            
            # 按钮内边距也相应缩小
            padding_v = int(10 * 0.9)
            padding_h = int(12 * 0.9)
    ...
    def delete_tool(self, tool, button):
        ...
        try:
            ...
            try:
                if button is not None:
                    button.objectName()  # 测试对象是否有效
                    button_valid = True
            except RuntimeError:
                button_valid = False
            
            if button_valid and button in self.parent.tool_buttons:
            ...
            elif button in self.parent.tool_buttons:
                # 按钮已失效，只清理字典引用
                del self.parent.tool_buttons[button]
