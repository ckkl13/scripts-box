...
class SearchResultsUI:
    ...
    def highlight_text(self, text, search_text):
        ...
        # 高亮匹配文本，使用黄色背景
        highlighted = pattern.sub(lambda m: f'<span style="background-color: #FFD700; color: #000000; padding: 1px 3px; border-radius: 2px;">{m.group(0)}</span>', text_html)
    ...
    def create_search_result_item(self, tool):
        ...
        # 应用高亮
        highlighted_name = self.highlight_text(tool["name"], search_text)
        name_label.setText(highlighted_name)
